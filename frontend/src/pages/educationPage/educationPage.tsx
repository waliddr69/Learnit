...
import MinimumDistanceSlider from "../coursespage/Slider";
...
function Education() {
  ...
  function getCourses() {
    fetch(
      `${
        import.meta.env.VITE_API_COURSE_URL
      }/getFilteredCourses?type=education&domain=` +
      ...
      .then((res) => {
        if (res.success && res.courses) {
          ...
          const lessonsCount = res.courses.map((course: { chapters: any[] }) =>
  ...
  return (
    <main className="min-h-vh">
      ...
      <button
        onClick={handleClick}
        className="flex filter font-semi gap-4 px-12 py-3 border-3 text-2xl items-center  rounded-3xl  sticky top-5 left-1/2 transform -translate-x-1/2 z-1 mt-5 text-[12px] px-2 py-3  squircle sm:px-12 sm:py-3 sm:text-[16px] md:px-12 md:py-3 md:text-[18px] lg:px-12 lg:py-3 lg:text-[18px]"
      >
      ...
      <div className="filters  items-center  w-1/2 overflow-x-scroll hidden sticky py-6 opacity-0  top-5 left-1/2 transform   rounded-3xl    -translate-x-1/2 z-50 mt-5 ">
        <div className="filter-dif flex flex-col gap-4 items-center px-4">
          <p className="font-medium text-lg whitespace-nowrap">Year :</p>
          <select name="dif" onChange={(e) => setDifficulty(e.target.value)}>
            {selected !== "Medecine" ? (
              <>
                <option value="All">All</option>
                <option value="1ère Année">1ère Année</option>
                <option value="2ère Année">2ère Année</option>
                <option value="3ère Année">3ère Année</option>
                <option value="4ère Année">4ère Année</option>
                <option value="5ère Année">5ère Année</option>
              ...
              <>
                ...
                <option value="4ère Année">4ère Année</option>
                <option value="5ère Année">5ère Année</option>
                <option value="6ère Année">6ère Année</option>
                <option value="7ère Année">7ère Année</option>
              </>
            )}
          </select>
        </div>
      ...
      <div className="courses-wrapper flex flex-col gap-15">
        <div className="domains flex gap-4 overflow-x-auto items-center font-medium sm:text-lg justify-start lg:justify-center mt-10">
          {Object.entries(domainsEd).map(([domain, icon], index) => {
            return (
              <DomainCard
                key={index}
                domain={domain}
                isselected={selected === domain ? true : false}
                onClick={() => {
                  isSelected(domain);
                }}
                icon={icon ?? undefined}
        ...
        <div className="relative flex flex-col mb-20">
          ...
          {courses.length > max && (
            <button
              ...
              className="load w-[90%] hover:outline-4 transition-all mt-10 p-5 rounded-3xl font-bold self-center outline-black outline-2"
            >
              Load more ....
            </button>
          )}
