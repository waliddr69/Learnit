import {  ChevronRight, FileText, NotebookPen, PlaySquare, Plus, SquarePlay, Users } from 'lucide-react';
...
function CourseManage(){
    ...
    const [payement, setPayement] = useState<string>("");
        ...
        function handleAddChapter(e: React.FormEvent<HTMLFormElement>) {
            ...
            const chapter = form.querySelector('input[name="chapter"]') as HTMLInputElement;
            if (chapter && chapter.value.trim() !== "") {
                showChapters([...chapters, {chapter: chapter.value.trim(), lessons: []}]);
                chapter.value = "";
        ...
        function handleAddLesson(e: React.FormEvent<HTMLFormElement>) {
            e.preventDefault();
            const form = e.currentTarget;
            const lesson = form.querySelector('input[name="lesson"]') as HTMLInputElement;
            console.log(lesson.value);
            const contentInput = form.querySelector('input[id="content"]') as HTMLInputElement;
        ...
        function handleContentChange(e: React.ChangeEvent<HTMLInputElement>) {
            ...
            if(file.type == "application/pdf"){ setContentType("pdf"); return "pdf"}
            if(file.type.startsWith("video/")) { setContentType("video"); return "video"}
        }
    ...
    return(
        <div className="manage flex flex-col gap-12">
            <div className='flex head flex-row justify-between'><h3>ML for beginners</h3> <div className="status text-black px-5 bg-[#F5BB62] rounded-3xl w-[10%] flex  items-center justify-center">Draft</div></div>
                      <p style={{color:"#1F1CD9",fontWeight:"600", width:"100%"}} > <span style={{color:"#1f1cd99c"}}>Tech /</span>  Web Dev </p>  
            ...
            <div className="stats flex flex-row flex-wrap lg:flex-nowrap justify-between gap-4">
                ...
                <div className="stat-card flex-1 flex flex-col p-4 gap-2 border-2 rounded-3xl border-[#08203e76]">
                    <div className='flex flex-row gap-2 items-center'><Wallet className='bg-[#F9D6E5] p-2 rounded-3xl' size={40}/><p>Revenue</p></div>
                ...
                <div className="stat-card flex-1 flex gap-2 flex-col p-4 border-2 rounded-3xl border-[#08203e76]">
                    <div className='flex flex-row gap-2 items-center'><div className='rounded-3xl overflow-hidden'><MessageSquareQuote className='bg-[#D5F3F0] p-2 ' size={40}/></div><p>Total reviews</p></div>
                    <h4 className='font-semibold'>278</h4>
                    <button  className="view-link cursor-pointer self-end" onClick={()=>showReviews(true)}>View &gt;</button>
                </div>
                <div className="stat-card flex-1 flex gap-2 flex-col p-4 border-2 rounded-3xl border-[#08203e76]">
                    <div className='flex flex-row gap-2 items-center'><Star className='bg-[#FCEBD2] p-2 rounded-3xl' size={40}/><p>Course rating</p></div>
            ...
            <div className='flex flex-col gap-4'>
                ...
                {chapters.map((chapter) => {
                    ...
                    return (
                        ...
                        <div
                            ...
                            className="chapter py-4 px-2 flex gap-2 border-b-2 cursor-pointer"
                        >
                        ...
                        {isOpen && (
                            <div className="chapter-content px-4 py-3 bg-white flex flex-col gap-3 border-b rounded-b-3xl border-1">
                            {chapter.lessons.length > 0 ? (
                                chapter.lessons.map((lesson) => (
                                    
                                <div key={lesson.title} className="flex gap-2 items-center">
                                    {lesson.content === "video" ? <PlaySquare/> : <FileText/>}
                                    <p>{lesson.title}</p>
                                </div>
                                ))
                            ) : (
                                <button onClick={()=>{setActiveChapter(chapter.chapter);showLessonInp(true); }} className="text-sm text-blue-600">
                                + Add first lesson
                                </button>
                            )}
                            </div>
                ...
                <button onClick={()=>showChapterinp(true)} className='flex  flex-row gap-2 items-center justify-center py-2 px-5 text-white rounded-3xl cursor-pointer bg-[#10305A] w-[20%]'><Plus /><p>Add Chapter</p></button>
            </div>
            <div>
            ...
            <div className="flex flex-col gap-8 w-full  items-center flex-wrap justify-center  lg:flex-nowrap mt-10 ">
                    <form action="" className="flex px-2 flex-col gap-6 w-full">
                        ...
                        <div className="form-group flex flex-col gap-2 w-full  ">
                            <label htmlFor="" className="text-[#333333] font-bold">Course Status (once changed to "published" the course will be shared in the platform to be seen by learners !)</label>
                            <div className="flex flex-row gap-2 w-full font-medium"><input  value={"draft"}  type="radio" name="payement" className=" w-12 p-4 border-2 border-[#0c24436d] rounded-sm"/> Draft</div>
                            <div className="flex flex-row gap-2 w-full font-medium"><input  value={"published"}  type="radio" name="payement"  className=" w-12 p-4 border-2 border-[#0c24436d] rounded-sm"/> Published</div>
                           
                        </div>
                        <div className="form-group flex flex-col gap-2 w-full  ">
                            <label htmlFor="" className="text-[#333333] font-bold">Course difficulty</label>
                            <select name="" id="" className=" w-full p-4 border-2 border-[#0c24436d] rounded-sm">
            ...
            <div className={` fixed ${chapterinp?"flex":"hidden"} items-center rounded-3xl z-20 bg-white p-4  left-1/2 transform translate-y-1/2 -translate-x-1/2 flex-col gap-6 `}>
                <h5 className='font-bold'>Add chapter</h5>
                <form action="" onSubmit={handleAddChapter} className='flex flex-col gap-5'>
                    <input type="text" placeholder='Chapter name' name='chapter'/>
                    <button type='submit' className='bg-[#10305A] cursor-pointer text-white px-4 py-2 rounded-3xl'>Add</button>
                </form>
            </div>
            <div className={` fixed ${lessoninp?"flex":"hidden"} items-center rounded-3xl z-20 bg-white p-4 w-[80%] sm:w-[50%]   left-1/2 transform top-0 bottom-0 -translate-x-1/2 flex-col gap-6 `}>
                <h5 className='font-bold'>Add Lesson</h5>
                <form action="" onSubmit={handleAddLesson} className='flex flex-col w-full overflow-y-auto gap-5'>
                    <input onChange={(e)=>setLessonTitle(e.target.value)} type="text" placeholder='lesson name' className='w-full p-4 border-2 border-[#0c24436d] rounded-sm' name='lesson'/>
                    <div className={` form-group flex  flex-col gap-2 w-full  `}>
                            <p className="text-[#333333] font-bold">lesson content (could be a video, or a pdf)</p>
                            <label htmlFor="content" className="text-[#333333] relative z-20 w-full box font-bold">{!content&&<Plus/>}{content && <object data={content}   className="preview relative z-10" />}</label>
                            
                           <input type="file" id="content" onChange={(e)=>handleContentChange(e)}   className="w-full hidden p-4 border-2 border-[#0c24436d] rounded-sm"/> 
            ...
            <div className={`reviews fixed ${reviews?"flex":"hidden"} z-20 bg-white p-4 top-0 bottom-0 left-1/2 transform -translate-x-1/2 flex-col gap-6 `}>
                        <div className="flex gap-2 items-center"><MessageSquareQuote width={30} height={30}/><h5 className="font-semibold">Reviews</h5> <X className='self-end cursor-pointer ml-auto' onClick={()=>showReviews(false)}/></div>
                        <p className="text-[#006FFF]">270 reviews</p>
                        <div className="overflow-auto" style={{maxHeight:'450px',scrollbarWidth:"none"}}>
                    ...
                    <div className={` ${reviews||chapterinp||lessoninp?"block":"hidden"} bg-[rgba(0,0,0,0.4)]  fixed inset-0`} onClick={()=>{showReviews(false);showChapterinp(false);showLessonInp(false)}}></div>
