...
function CoursesCard({
  ...
  useEffect(() => {
    if (reviewsCs?.length! > 0) {
      const total = reviewsCs?.reduce(
        (acc, curr: ReviewsC) => acc + curr.rating,
        0,
      );
      setReviews(total! / reviewsCs!.length);
      return;
    }
  }, []);
  ...
  return (
    <div
      ...
      className="card p-2  min-w-80 lg:min-w-90 group    border-2 bg-[#F8FAFF] border-[#E1E2F3] rounded-3xl   flex flex-col gap-4"
    >
      <div
        className="thumbnail relative rounded-3xl"
        style={{
          backgroundImage: `url(${import.meta.env.VITE_API_FILE_URL}/${photo})`,
        }}
      >
      ...
      <div className="px-2 flex flex-col gap-4">
        ...
        <div className="teacher flex gap-2 items-center">
          <div
            className="teacher-img w-8 h-8 rounded-3xl"
            style={{
        ...
        <div className="rating flex flex-row gap-1 items-center">
          ...
          <p className="font-semibold">
            <span className="text-2xl">{reviews}</span>/5 ({reviewsCs?.length}{" "}
            reviews)
          </p>
        </div>
        <div className="course-info items-center  gap-2 flex justify-between">
          <div className="difficulty-course items-center  gap-3 flex-col     rounded-3xl flex border-[#00c93257]">
            <p
              className={`text-green-400 font-medium px-2 border-2 border-[${color}] rounded-3xl`}
        ...
        <div className="main-card-info flex justify-between items-center px-2 mt-4">
          ...
          {!isEnrolled && (
            <button
              onClick={() => history("/course/" + id)}
              className="enroll-btn cta cta main-btn text-[12px] px-1 py-3  squircle sm:px-5 sm:py-3 sm:text-[16px] md:px-5 md:py-3 md:text-[18px] lg:px-5 lg:py-3 lg:text-[18px]"
            >
              Enroll Now
            </button>
          )}
